...
namespace ThrottlR
{
    ...
    internal sealed class AsyncKeyLock
    {
        ...
        /// </summary>
        private static readonly Dictionary<string, AsyncKeyLockDoorman> _keys = new Dictionary<string, AsyncKeyLockDoorman>();

        /// <summary>
        /// A pool of unused doorman counters that can be re-used to avoid allocations.
        /// </summary>
        private static readonly Stack<AsyncKeyLockDoorman> _pool = new Stack<AsyncKeyLockDoorman>(MaxPoolSize);
        ...
        private static AsyncKeyLockDoorman GetDoorman(string key)
        {
            ...
            try
            {
                ...
                {
                    doorman = (_pool.Count > 0) ? _pool.Pop() : new AsyncKeyLockDoorman(ReleaseDoorman);
                    doorman.Key = key;
